...
parser = argparse.ArgumentParser(description="motifAnalysis.py: use this to look for motifs in CLIP data", epilog="Example: python motifAnalysis.py ddx3wt --hg19 -l 12,14")
...
parser.add_argument('-l', metavar='LENGTHS', help="Length(s) of motifs desired; separated by a ','", required=True)
...
def extractClusters(geneList,allClusters):
    ...
    # BD Update 12/13: only unique clusters
    extractedClusters = geneList+'clusters.bed'
    outfh = open(extractedClusters, 'w')
    writer = csv.writer(outfh, 'textdialect')
    
    # List of allowed gene names
    namesToQuery = np.genfromtxt(geneList,usecols=(0,),delimiter='\t',dtype='string')
    
    # Iterate through clusters
    ifile = open(allClusters, 'r')
    reader = csv.reader(ifile, 'textdialect')
    
    uniqueClusters = set()
    for row in reader:
        geneName = row[3].split('_')[0]
        if geneName not in namesToQuery: continue
        if '__'.join(row[:3]) not in uniqueClusters: 
            writer.writerow(row)
            uniqueClusters.add(('__').join(row[:3]))     
